...
describes what users get. Pick a format with `-o`, or let xiaoyuzhou choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
xiaoyuzhou <command> -o table   # aligned columns for reading
xiaoyuzhou <command> -o jsonl   # one JSON object per line, for piping
xiaoyuzhou <command> -o json    # a single JSON array
xiaoyuzhou <command> -o csv     # spreadsheet friendly
xiaoyuzhou <command> -o tsv     # tab-separated
xiaoyuzhou <command> -o url     # just the URL column
xiaoyuzhou <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
xiaoyuzhou <command> --fields id,title,url
```
...
```bash
xiaoyuzhou <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
xiaoyuzhou <command>            # a table, because this is a terminal
xiaoyuzhou <command> | wc -l    # JSONL, because this is a pipe
```
